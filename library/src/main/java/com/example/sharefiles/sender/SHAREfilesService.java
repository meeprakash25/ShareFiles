...
import static com.example.sharefiles.sender.SHAREfilesService.ShareIntents.SHARE_CLIENT_IP;
import static com.example.sharefiles.sender.SHAREfilesService.ShareIntents.SHARE_SERVER_UPDATES_INTENT_ACTION;
import static com.example.sharefiles.sender.SHAREfilesService.ShareIntents.SHARE_SERVER_UPDATE_FILE_NAME;
import static com.example.sharefiles.sender.SHAREfilesService.ShareIntents.SHARE_SERVER_UPDATE_TEXT;
import static com.example.sharefiles.sender.SHAREfilesService.ShareIntents.SHARE_TRANSFER_PROGRESS;
import static com.example.sharefiles.sender.SHAREfilesService.ShareIntents.TYPE;
import static com.example.sharefiles.sender.SHAREfilesService.ShareIntents.Types.AP_DISABLED_ACKNOWLEDGEMENT;
import static com.example.sharefiles.sender.SHAREfilesService.ShareIntents.Types.FILE_TRANSFER_STATUS;


/**
 * Manages Hotspot configuration using an instance of {@link HotspotControl} and also deals lifecycle of {@link SHAREfilesServer}
 * Created by Sri on 18/12/16.
 */
public class SHAREfilesService extends Service {

    private static final String TAG = "ShareService";

    private WifiManager wifiManager;
    private HotspotControl hotspotControl;
    private SHAREfilesServer m_fileServer;
    ...
    public void onCreate() {
        ...
        startForeground(SHARE_SERVICE_NOTIFICATION_ID, getShareFilesNotification(getString(R.string.p2p_sender_service_init_notification_header), false));
...
//                    sendAcknowledgementBroadcast(AP_ENABLED_ACKNOWLEDGEMENT);
     ...
     * @param addStopAction if true STOP action is added to stop {@link SHAREfilesService}
     * @return Notification
     */
    private Notification getShareFilesNotification(String text, boolean addStopAction) {
        Intent notificationIntent = new Intent(getApplicationContext(),
                SHAREfilesActivity.class);
    ...
    /**
     * Starts {@link SHAREfilesServer} and then enables Hotspot with assigned port being part of encoded SSID.
     *
    ...
    private void startFileHostingServer(String[] filePathsTobeServed, int port, final String sender_name) {
        m_fileServer = new SHAREfilesServer(getApplicationContext()
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    hotspotControl.enableShareFilesHotspot(name, m_fileServer.getListeningPort());
    ...
    private void updateForegroundNotification() {
        ...
        mNotificationManager.notify(SHARE_SERVICE_NOTIFICATION_ID, getShareFilesNotification(getString(R.string.p2p_sender_service_notification_header), true));
    }

    private static class HotspotChecker extends Handler {
        WeakReference<SHAREfilesService> service;

        HotspotChecker(SHAREfilesService senderService) {
            this.service = new WeakReference<>(senderService);
        }

        @Override
        public void handleMessage(Message msg) {
            SHAREfilesService senderService = service.get();
