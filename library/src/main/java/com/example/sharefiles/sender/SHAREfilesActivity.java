 ...
 */
package com.example.sharefiles.sender;
...
import com.example.sharefiles.R;
import com.example.sharefiles.utils.DividerItemDecoration;
import com.example.sharefiles.utils.RecyclerViewArrayAdapter;
import com.example.sharefiles.utils.Utils;
import com.example.sharefiles.utils.HotspotControl;
import com.example.sharefiles.utils.WifiUtils;
...
import static com.example.sharefiles.sender.SHAREfilesActivity.ShareUIHandler.LIST_API_CLIENTS;
import static com.example.sharefiles.sender.SHAREfilesActivity.ShareUIHandler.UPDATE_AP_STATUS;
import static com.example.sharefiles.utils.Utils.getRandomColor;
...
public class SHAREfilesActivity extends AppCompatActivity {
     ...
     */
    public static final String PREFERENCES_KEY_SHARED_FILE_PATHS = "sharefiles_shared_file_paths";
    public static final String PREFERENCES_KEY_DATA_WARNING_SKIP = "sharefiles_data_warning_skip";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (null != getIntent() && getIntent().hasExtra(SHAREfilesService.EXTRA_FILE_PATHS))
            m_sharedFilePaths = getIntent().getStringArrayExtra(SHAREfilesService.EXTRA_FILE_PATHS);
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (isChecked) {
                    ...
                    if (Build.VERSION.SDK_INT >= 23 &&
                            ...
                            Utils.getTargetSDKVersion(getApplicationContext()) >= 23 && !Settings.System.canWrite(SHAREfilesActivity.this)) {
        ...
        m_p2pServerUpdatesListener = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                if (isFinishing() || null == intent)
                    return;
                int intentType = intent.getIntExtra(SHAREfilesService.ShareIntents.TYPE, 0);
                if (intentType == SHAREfilesService.ShareIntents.Types.FILE_TRANSFER_STATUS) {
                    String fileName = intent.getStringExtra(SHAREfilesService.ShareIntents.SHARE_SERVER_UPDATE_FILE_NAME);
                    updateReceiverListItem(intent.getStringExtra(SHAREfilesService.ShareIntents.SHARE_CLIENT_IP), intent.getIntExtra(SHAREfilesService.ShareIntents.SHARE_TRANSFER_PROGRESS, -1), intent.getStringExtra(SHAREfilesService.ShareIntents.SHARE_SERVER_UPDATE_TEXT), fileName);
                } else if (intentType == SHAREfilesService.ShareIntents.Types.AP_DISABLED_ACKNOWLEDGEMENT) {
                    shouldAutoConnect = false;
                    resetSenderUi(false);
                }
            }
        };
        registerReceiver(m_p2pServerUpdatesListener, new IntentFilter(SHAREfilesService.ShareIntents.SHARE_SERVER_UPDATES_INTENT_ACTION));
    }
    ...
    private void disableAp() {
        ...
        Intent p2pServiceIntent = new Intent(getApplicationContext(), SHAREfilesService.class);
        p2pServiceIntent.setAction(SHAREfilesService.WIFI_AP_ACTION_STOP);
        startService(p2pServiceIntent);
        isApEnabled = false;
    }

    /**
     * Starts {@link SHAREfilesService} with intent action {@link SHAREfilesService#WIFI_AP_ACTION_START} to enableShareFilesHotspot Hotspot and start {@link SHAREfilesServer}.
     */
    private void startP2pSenderWatchService() {
        Intent p2pServiceIntent = new Intent(getApplicationContext(), SHAREfilesService.class);
        p2pServiceIntent.putExtra(SHAREfilesService.EXTRA_FILE_PATHS, m_sharedFilePaths);
        if (null != getIntent()) {
            p2pServiceIntent.putExtra(SHAREfilesService.EXTRA_PORT, getIntent().getIntExtra(SHAREfilesService.EXTRA_PORT, 0));
            p2pServiceIntent.putExtra(SHAREfilesService.EXTRA_SENDER_NAME, getIntent().getStringExtra(SHAREfilesService.EXTRA_SENDER_NAME));
        }
        p2pServiceIntent.setAction(SHAREfilesService.WIFI_AP_ACTION_START);
        startService(p2pServiceIntent);
    }

    /**
     * Starts {@link SHAREfilesService} with intent action {@link SHAREfilesService#WIFI_AP_ACTION_START_CHECK} to make {@link SHAREfilesService} constantly check for Hotspot status. (Sometimes Hotspot tend to stop if stayed idle for long enough. So this check makes sure {@link SHAREfilesService} is only alive if Hostspot is enaled.)
     */
    private void startHostspotCheckOnService() {
        Intent p2pServiceIntent = new Intent(getApplicationContext(), SHAREfilesService.class);
        p2pServiceIntent.setAction(SHAREfilesService.WIFI_AP_ACTION_START_CHECK);
        startService(p2pServiceIntent);
    }

    /**
     * Calls methods - {@link SHAREfilesActivity#updateApStatus()} & {@link SHAREfilesActivity#listApClients()} which are responsible for displaying Hotpot information and Listing connected clients to the same
     */
     ...
     * Constantly adds/updates receiver items on {@link SHAREfilesActivity#m_receiversList}
    ...
    /**
     * Adds a {@link HotspotControl.WifiScanResult} item to {@link SHAREfilesActivity#m_receiversListAdapter} if not already added
     *
...
//            m_receiversListAdapter.remove(new WifiApControl.Client(ip, null, null));
    ...
    static class ShareUIHandler extends Handler {
        WeakReference<SHAREfilesActivity> mActivity;

        static final int LIST_API_CLIENTS = 100;
        static final int UPDATE_AP_STATUS = 101;

        ShareUIHandler(SHAREfilesActivity activity) {
        ...
        public void handleMessage(Message msg) {
            ...
            SHAREfilesActivity activity = mActivity.get();
