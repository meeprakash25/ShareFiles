 ...
 */
package com.example.sharefiles.receiver;
...
import static com.example.sharefiles.receiver.ReceiverActivity.WifiTasksHandler.SCAN_FOR_WIFI_RESULTS;
import static com.example.sharefiles.receiver.ReceiverActivity.WifiTasksHandler.WAIT_FOR_CONNECT_ACTION_TIMEOUT;
import static com.example.sharefiles.receiver.ReceiverActivity.WifiTasksHandler.WAIT_FOR_RECONNECT_ACTION_TIMEOUT;
import static com.example.sharefiles.utils.WifiUtils.connectToOpenHotspot;
...
public class ReceiverActivity extends AppCompatActivity {
    ...
    public void onResume() {
        super.onResume();
        boolean isConnectedToShareFilesAp = WifiUtils.isWifiConnectedToSTAccessPoint(getApplicationContext());
        if (isConnectedToShareFilesAp) {
            unRegisterForScanResults();
            if (!m_receiver_control.isChecked())
                changeReceiverControlCheckedStatus(true);
            String ssid = wifiManager.getConnectionInfo().getSSID();
            Log.d(TAG, "wifi is connected/connecting to ShareFiles ap, ssid: " + ssid);
            mConnectedSSID = ssid;
            addSenderFilesListingFragment(WifiUtils.getAccessPointIpAddress(this), ssid);
        } else if (m_receiver_control.isChecked()) {
            Log.d(TAG, "wifi isn't connected to ShareFiles ap, initiating sender search..");
            resetSenderSearch();
        }
    }
    ...
    /**
     * Entry point to start receiver mode. Makes calls to register necessary broadcast receivers to start scanning for ShareFiles Wifi Hotspot.
     *
     * @return
     */
    ...
    /**
     * Disables and removes SHAREfiles wifi configuration from Wifi Settings. Also does cleanup work to remove handlers, un-register receivers etc..
     */
    ...
    private void connectToWifi(String ssid) {
        ...
        if (info.getSSID().equals(ssid)) {
            Log.d(TAG, "Already connected to ShareFiles, add sender Files listing fragment");
            resetWifiScan = false;
            ...
            Log.e(TAG, "connection attempt to ShareFiles wifi is " + (!resetWifiScan ? "success!!!" : "FAILED..!!!"));
        }
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(WifiManager.SCAN_RESULTS_AVAILABLE_ACTION) && !WifiUtils.isWifiConnectedToSTAccessPoint(getApplicationContext())) {
                ...
                for (ScanResult result : mScanResults)
                    if (WifiUtils.isShareFilesSSID(result.SSID) && WifiUtils.isOpenWifi(result)) {
                ...
                if (ConnectivityManager.TYPE_WIFI == netInfo.getType()) {
                    ...
                    if (WifiUtils.isShareFilesSSID(info.getSSID())) {
                        if (System.currentTimeMillis() - preferences.getLong(LASTCONNECTEDTIME, 0) >= SYNCTIME && supState.equals(SupplicantState.COMPLETED)) {
                            ...
                            Log.d(TAG, "client connected to ShareFiles hot spot. AP ip address: " + ip);
            ...
            switch (msg.what) {
                ...
                case WAIT_FOR_RECONNECT_ACTION_TIMEOUT:
                    if (WifiUtils.isWifiConnectedToSTAccessPoint(activity) || activity.isFinishing())
                        return;
                    Log.e(TAG, "Even the dirty hack couldn't do it, prompt user to chose it from WIFI settings..");
